...
for f in connmand NetworkManager wicd; do
	...
	echo "Shuting off dhcpcd becuase of $f"
        unset dhcpcd
    fi
done
...
for f in $SERVICEDIR/*; do
    _service=${f##*/}
    case "${_service}" in agetty-console|agetty-generic|agetty-serial|agetty-tty[SAU]*|sulogin|dhcpcd-*|iptables|ip6tables|wpa_supplicant|pulseaudio|lvmetad|dmeventd|mdadm|sshd|lxdm) ;; # ignored
